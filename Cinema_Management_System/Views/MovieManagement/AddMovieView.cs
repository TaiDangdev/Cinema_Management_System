...
namespace Cinema_Management_System.Views.MovieManagement
{
    public partial class AddMovieView : Form
    {
        ...
        private void ReadCSV(string filePath)
        {
            ...
            {
                 MessageBoxHelper.ShowError("Lỗi", $"Lỗi khi đọc file: {ex.Message}");
            }
        }
        ...
        private void BindMovieToForm(MovieData movie)
        {
            ...
            statusMovie_Txt.Text = movie.Status;
            priceMovie_Txt.Text = movie.Price.ToString("N0");

            if (DateTime.TryParseExact(movie.ReleaseDate, "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out var date))
                startDateMovie_Txt.Value = date;
        }
        ...
        private void ReadExcel(string filePath)
        {
            ...
            {
                MessageBoxHelper.ShowError("Lỗi", $"Lỗi khi đọc file Excel: {ex.Message}");
            }
        }
        ...
        private MovieData ExtractMovieFromExcel(IXLWorksheet worksheet, int row)
        {
            return new MovieData
            {
                ...
                Status = worksheet.Cell(row, 11).GetString(),
                Price = decimal.TryParse(worksheet.Cell(row, 12).GetString(), out var price) ? price : 0,
                ReleaseDate = worksheet.Cell(row, 13).GetString()
            };
        }
        ...
        public class MovieData
        {
            ...
            [Name("Tình trạng phim")] public string Status { get; set; } 
